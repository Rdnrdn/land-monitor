"""Simple CLI router for Telegram/OpenClaw command bridge."""
...
def main(argv: list[str]) -> int:
    if len(argv) < 2:
        print("Usage: bot_router.py <status|recent|cheapest|auction|runs> [args]")
        return 1
    ...
    if command == "auction":
        if len(argv) < 3:
            print("Usage: bot_router.py auction <id>")
            return 1
